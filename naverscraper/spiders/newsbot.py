...
class NewsbotSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for idx in range(len(titles)):
            ...
            item['price'] = int(prices[idx].replace(',',''))
            ...
            item['volume'] = int(volumes[idx].replace(',',''))
            item['payment'] = int(payments[idx].replace(',',''))
            item['buy'] = int(buys[idx].replace(',',''))
            item['sell'] = int(sells[idx].replace(',',''))
